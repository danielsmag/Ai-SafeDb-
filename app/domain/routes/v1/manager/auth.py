"""Manager UI authentication routes."""
...
def register_manager_auth_routes(api: FastAPI, ctx: GatewayContext) -> None:
    store: AuthStore | None = ctx.auth_store
    if store is None:
        return

    @api.post(
        f"{MANAGER_API_PREFIX}/login",
    ...
    @api.post(f"{MANAGER_API_PREFIX}/logout", status_code=204, tags=["manager-ui"])
    async def manager_logout(request: Request, response: Response) -> None:
    ...
    @api.get(
        f"{MANAGER_API_PREFIX}/me",
        response_model=UserIdentityResponse,
        tags=["manager-ui"],
    )
    async def manager_get_identity(request: Request) -> UserIdentityResponse:
